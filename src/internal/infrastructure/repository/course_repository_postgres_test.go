...
func setupCourseTestDB(t *testing.T) (*gorm.DB, *entity.User) {
	...
	if err != nil {
		t.Skipf("skipping course repo tests: %v", err)
	}
...
func TestPostgresCourseRepository_List(t *testing.T) {
	...
	courses, total, err := repo.List(context.Background(), &domainRepository.CourseFilter{Limit: 10, Offset: 0})
	assert.NoError(t, err)
	assert.NotNil(t, courses)
	assert.GreaterOrEqual(t, len(courses), 2)
	assert.Equal(t, total, int64(len(courses)))
}
...
func TestPostgresCourseRepository_List_WithFilters(t *testing.T) {
	...
	t.Run("Filter by InstructorID", func(t *testing.T) {
		...
		courses, total, err := repo.List(context.Background(), &domainRepository.CourseFilter{InstructorID: &instructorIDStr})
		assert.NoError(t, err)
		assert.Len(t, courses, 2)
	...
	t.Run("Filter by DifficultyLevel", func(t *testing.T) {
		...
		courses, total, err := repo.List(context.Background(), &domainRepository.CourseFilter{DifficultyLevel: &difficulty})
		assert.NoError(t, err)
	...
	t.Run("Filter by ActiveOnly", func(t *testing.T) {
		courses, total, err := repo.List(context.Background(), &domainRepository.CourseFilter{ActiveOnly: true})
		assert.NoError(t, err)
		assert.Len(t, courses, 2)
		assert.Equal(t, int64(2), total)
	})
...
func TestPostgresCourseRepository_List_WithPagination(t *testing.T) {
	...
	// Get first page
	courses, total, err := repo.List(context.Background(), &domainRepository.CourseFilter{Limit: 1, Offset: 0})
	assert.NoError(t, err)
	assert.Len(t, courses, 1)
	...
	// Get second page
	courses, total, err = repo.List(context.Background(), &domainRepository.CourseFilter{Limit: 1, Offset: 1})
	assert.NoError(t, err)
	assert.Len(t, courses, 1)
	assert.Equal(t, course1.ID, courses[0].ID)
	assert.Equal(t, int64(2), total)
